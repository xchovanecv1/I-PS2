/*
 ...
 Zachytte udalost (PhyRxDrop || PhyTxDrop) triedy .... funkciou, kde (1b)
  vykreslite graf(<ktory uzol dropol>,t) ; t je cas simulacie (2b)
 * 
 * 
 * Viktor Chovanec 80331
*/
...
void WalkNodes(double speed) {
    
    for(int i = 5; i <= 7; i++) {
        std::stringstream ss;
        ss << "NodeList/" << i << "/$ns3::MobilityModel/$ns3::RandomWalk2dMobilityModel/Speed";
        std::string s = ss.str();
        
        std::stringstream ssval;
        ssval << "ns3::ConstantRandomVariable[Constant="<< speed << "]";
        std::string sval = ssval.str();
        
        Config::Set(s,StringValue(sval));
    }
...
void static cwdn(uint32_t oldVal, uint32_t newVal) {
    
    if(oldVal > newVal) {
        nodeSpeed++;
        WalkNodes(nodeSpeed);
        cout << "Up " << nodeSpeed << endl;
    } else {
        nodeSpeed /= 2;
        WalkNodes(nodeSpeed);
        cout << "Down " << nodeSpeed << endl;
    }
    std::cout << "Traced " << oldVal << " " << newVal << std::endl;

}

void rxdrop(string context, Ptr< const Packet > pak) {
    
    std::stringstream test(context);
    std::string segment;
    ...
    data.Add(Simulator::Now().GetSeconds(), nodeID);
    
    //cout << "RX DROP " <<context << " Node " << nodeID << endl;
}
...
int main (int argc, char *argv[]){
  ...
  Ptr<FlowMonitor> monitor = flowmon.InstallAll();
   ...
   Simulator::Schedule (Seconds (20), &WalkNodes, nodeSpeed);
  Config::ConnectWithoutContext("/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/CongestionWindow",MakeCallback(&cwdn)); 
  Config::Connect("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyRxDrop",MakeCallback(&rxdrop)); 
...
*/
  ...
  graf.SetLegend("Cas","Cislo nodu");
  graf.SetTitle("RX Drop nodov v case");

  data.SetStyle (Gnuplot2dDataset::POINTS);
