...
- Zachytte udalost zmeny vzdialenej kontroly okna (RWND), 
pomocka: RWND je nazov udalosti v triede TcpSocket, tu mozno zavolat az ked je socket vytvoreny t.j. v case simulacie. Bud priamo cez objekt typu Socket, kt. mozno ziskat z aplikacie.
app->GetObject<OnOffAplication>()->GetSocket(); na ktorom existuje clenska funkcia umoznenie zachytavania udalosti.
(1b)
   vykreslite graf zavislosti velkosti vzdialenej kontroly v case. (1b)
-- Ak nastane pokles vzdialenej kontroly, zmente na vsetkych uzloch aktivny cas cesty balikov ActiveRouteTimeout, aby sa cesta nevyprsala tak skoro
...
void RwndChange(uint32_t oldValue, uint32_t newValue){
    cout << "zachytene" << endl;
    data.Add(Simulator::Now().GetSeconds(), newValue);
    if(newValue<oldValue){
        Config::Set("/NodeList/*/$ns3::aodv::RoutingProtocol/ActiveRouteTimeout", TimeValue(Seconds(1)));
    }
}

static void CasovaUdalost(Ptr<Application> app){
    cout << "zachytena casovaudalost" << endl;
    app->GetObject<OnOffApplication>()->GetSocket()->TraceConnectWithoutContext("RWND", MakeCallback(&RwndChange));
}
...
int main (int argc, char *argv[]){
  ...
  Simulator::Schedule(Seconds(1+0.0001), &CasovaUdalost, clientAppA.Get(0));
  ...
  graf.SetLegend("Cas","RWND");
  graf.SetTitle("Zavislost RWND od casu");
