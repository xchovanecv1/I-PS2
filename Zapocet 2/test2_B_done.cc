...
- Zachytte udalost zmeny casu cesty segmentu (RTT), (*pomocka) (1b)
   vykreslite graf zavislosti velkosti casu cesty segmentu v case. (1b)
-- Ak nastane zvysenie velkosti casu cesty, zmente na vsetkych uzloch rychlost pohybu (nahodny vyber z rozsahu -- Speed) ns3::UniformRandomVariable[Min=2.0|Max=4.0](2b)
*/
...
void static callback(Time v, Time w) {
    //std::cout << "Traced ";
    data.Add(Simulator::Now().GetSeconds(), w.GetSeconds());
    
    if (v.GetSeconds() < w.GetSeconds()) {  
        //Config::Set("/NodeList/*/$ns3::MobilityModel/$ns3::RandomWalk2dMobilityModel/Speed", StringValue("ns3::UniformRandomVariable[Min=2.0|Max=4.0]"));
    }
...
void CasovaUdalost(Ptr<Application> app){
        ...
        app->GetObject<OnOffApplication>()->GetSocket()->TraceConnectWithoutContext ("RTT", MakeCallback (&callback));
}
...
int main (int argc, char *argv[]){
  ...
  clientAppA.Stop (Seconds (simulationTime + 1));
   ...
   //casova udalost
  Simulator::Schedule (Seconds (1.0 + 0.001), &CasovaUdalost, clientAppA.Get(0));
  ...
  Simulator::Destroy ();
    ...
    graf.SetLegend("Cas","RTT");
    graf.SetTitle("Zavislost RTT od casu");
